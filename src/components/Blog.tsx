    ...
    >
      <Link href={`/blog/${post.slug}`} data-hover="true" className="block group">
        <div className="p-6 rounded-2xl border border-white/5 group-hover:border-white/12 transition-colors duration-300 h-full flex flex-col" style={{ background: "rgba(255,255,255,0.02)" }}>
          ...
          <h3 className="text-lg font-bold text-white/80 group-hover:text-white transition-colors duration-200 mb-3 leading-snug flex-1">
            {post.title}
          </h3>
          ...
          <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
            ...
            <span
              className="text-xs text-white/25 group-hover:text-[#ffaa33] transition-colors duration-200 flex items-center gap-1"
            >
              Read →
            </span>
          </div>
