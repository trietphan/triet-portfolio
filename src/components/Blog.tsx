...
function BlogCard({ post, index }: { post: (typeof posts)[0]; index: number }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.45, delay: index * 0.08, ease: [0.25, 0.1, 0.25, 1] as [number, number, number, number] }} onMouseMove={trackSpotlight}>
      <Link href={`/blog/${post.slug}`} data-hover="true" className="block group">
        <div className="spotlight p-6 rounded-2xl border border-white/5 group-hover:border-white/12 transition-colors duration-300 h-full flex flex-col" style={{ background: "rgba(255,255,255,0.02)" }}>
    ...
    <section id="blog" className="relative py-32 px-6">
      ...
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          ...
          <RevealHeading text="Thoughts &" accent="Writing"
            accentClass="bg-gradient-to-r from-[#b347ff] to-[#00fff5] bg-clip-text text-transparent"
